...
- `20260625-diff-差分文脈.md`: 草案。`context scope="diff"` でブランチの変更地図を返す方針。正本変更だけでなく変更ソースファイル一覧も含め、本文や生 diff は返さない。新規 diff 専用 tool は増やさない前提
...
- `20260625-skill-script化誘導.md`: 草案。routine 提案を `skill` / `script-skill` に分け、同じ手順3回以上・入力出力が決まる・終了コードで失敗を表せる等の条件を満たす機械処理を script 型 skill へ誘導する方針。生コマンドは保存せず、既存の `skill.register` / `skill.promote` を使う前提
...
- `20260625-tools-tool整理.md`: 草案。MCP tool の物理削除より先に、カテゴリ化・入口 skill の tool 実行順・短い description で AI の選択面を整理する方針。危険度や返り値が違う tool は統合せず、mode 統合は後回し
